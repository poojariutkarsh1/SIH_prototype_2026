...
// PWM SETTINGS
// =====================================================
// ESP8266 analogWrite() defaults to a 10-bit range (0-1023).
// We force it down to 0-255 so "speed" (0-100) maps simply.
...
void setup() {
  ...
  // Use an 8-bit PWM range (0-255) for simple speed mapping
  ...
  Serial.println("ACTUATOR SYSTEM READY (PWM)");
...
// =====================================================
// DRIVE FORWARD (PWM)
// speed is 0-100 (%), mapped to 0-PWM_MAX
// =====================================================
...
void driveForward(int speed) {
  ...
  int rightSpeed = speed + 2;
  int leftSpeed = speed;

  leftSpeed = constrain(leftSpeed, 0, 100);

  int leftPWM = map(leftSpeed, 0, 100, 0, PWM_MAX);
  int rightPWM = map(rightSpeed, 0, 100, 0, PWM_MAX);

  analogWrite(MOTOR_M1, leftPWM);
  analogWrite(MOTOR_M2, rightPWM);
...
//
// Kept at full digital drive (not PWM) - turning needs
// reliable torque from these low-torque BO motors, and
// it's a short timed maneuver rather than continuous driving.
// =====================================================
